const getFinalResult = (testResult = [], { maxPoint }) => {
	const status =
		...
		'AC';
	const time = testResult.some((test) => test.status == 'TLE') ? -1 : testResult.reduce((acc, test) => acc + test.time, 0);
	const memory = testResult.some((test) => test.status == 'MLE') ? -1 : testResult.reduce((acc, test) => Math.max(acc, test.memory), 0);
