...
const Sidebar = ({ user: initialUser }: Props) => {
	...
	const { data: user } = useSWR('/auth/me', () => authService.getProfile().catch(() => null), {
		fallbackData: initialUser,
		revalidateOnMount: true,
	});

	// Use SWR for other data with default error handling
	const { data: topUsersData, error: topUsersError } = useSWR('/users/top', () =>
		userService.getUsers({ sortBy: 'rating', sortOrder: 'DESC', limit: 3 }).then((res) => res.users),
	);

	const { data: hotProblemsData, error: hotProblemsError } = useSWR('/statistics/popular-problems', () => statisticsService.getPopularProblems(3).then((res) => res.data));

	const { data: scoreData, error: scoreError } = useSWR(user ? `/user/${user.username}/score` : null, () =>
			...
			{user && (
				<div className="bg-card rounded-2xl border p-6 shadow-xs">
					...
					<div className="flex gap-4">
						<div className="bg-background flex-1 rounded-lg border p-3 text-center">
							<h4 className="text-muted-foreground text-sm font-medium">{t('rating')}</h4>
							<p className="text-xl font-bold">{user.rating.at(-1) ?? 0}</p>
						</div>
						<div className="bg-background flex-1 rounded-lg border p-3 text-center">
							<h4 className="text-muted-foreground text-sm font-medium">{t('score')}</h4>
							<p className="text-xl font-bold">{score}</p>
						</div>
					</div>
				</div>
			)}
			{user?.joiningContest && getContestStatus(user.joiningContest) === ContestStatus.RUNNING && (
				<div className="bg-card rounded-2xl border p-6 shadow-xs">
					<h3 className="mb-4 text-lg font-semibold">{t('current_contest')}</h3>
					<div className="space-y-2">
						<Link href={`/contest/${user.joiningContest.slug}`} className="text-primary font-semibold hover:underline">
							{user.joiningContest.title}
						</Link>
						<p className="text-muted-foreground text-sm">
							{t('ending_in')} <ContestCountdown endTime={user.joiningContest.endTime} />
						</p>
					</div>
				</div>
			)}
