...
const CreateBlog = () => {
	...
	const onSave = () => {
		...
		if (!title || !content || content.content.length == 0) {
			toast.error(t('message.title-and-content-required'));
			return;
		}
	...
	return (
		<div className="max-w-app mx-auto my-6 h-full">
			...
			<div className="flex h-full flex-wrap gap-6">
				<div className="w-sm">
					<input ref={titleRef} type="text" className="peer w-full py-1 text-2xl font-medium outline-none" placeholder={t('title-placeholder')} />
					<div className="bg-border peer-focus:bg-ring mb-6 h-0.5 transition-all"></div>
					<ImageUpload ref={imageUploadRef} title={t('upload-thumbnail')} />
				</div>
				<div className="bg-card max-h-[calc(100vh-164px)] flex-1 rounded-xl border py-2 shadow-xs">
					<SimpleEditor onReady={(editor) => (editorRef.current = editor)} placeholder={t('description-placeholder')} />
				</div>
			</div>
		</div>
	);
