	...
	return (
		<>
			<div className="space-y-2">
				<h1 className="text-2xl leading-none font-semibold tracking-tight">{t('send_system_notification')}</h1>
				<p className="text-muted-foreground text-sm">{t('send_system_notification_description')}</p>
			</div>

			<form onSubmit={handleSubmit} className="space-y-4">
				<div className="space-y-2">
					<Textarea placeholder={t('content_placeholder')} value={content} onChange={(e) => setContent(e.target.value)} rows={6} disabled={isSubmitting} />
				</div>
