...
export function BasicPagination({ totalPages = 10, initialPage = 1, siblingsCount = 1, onPageChange, className, variant = 'default', showDemo = false }: BasicPaginationProps) {
	...
	const getButtonStyle = (isActive: boolean) => {
		if (variant === 'outline') {
			return isActive ? 'border-primary !text-primary-foreground !bg-primary' : 'border-border hover:border-primary/50 hover:text-primary';
		}

		if (variant === 'rounded') {
			return isActive ? 'bg-primary text-primary-foreground rounded-full' : 'hover:bg-muted rounded-full';
		}
	...
	const PaginationComponent = (
		<Pagination className={className}>
			<PaginationContent>
				<PaginationItem>
					<motion.div initial="initial" animate="animate" exit="exit" whileHover="hover" variants={itemVariants} transition={{ duration: 0.3 }}>
						<PaginationPrevious
							href="#"
							...
							className={cn(currentPage <= 1 ? 'pointer-events-none opacity-50' : '', variant === 'rounded' ? 'rounded-full' : '')}
						/>
					</motion.div>
				</PaginationItem>
					...
					if (page === 'leftEllipsis' || page === 'rightEllipsis') {
						return (
							<PaginationItem key={`ellipsis-${index}`}>
								<motion.div initial="initial" animate="animate" exit="exit" variants={itemVariants} transition={{ duration: 0.3, delay: index * 0.05 }}>
									<PaginationEllipsis />
								</motion.div>
							</PaginationItem>
						);
						...
						<PaginationItem key={pageNum}>
							...
							>
								<PaginationLink
									...
									className={cn(getButtonStyle(isActive), variant === 'outline' && 'border', 'transition-all duration-200')}
								>
									{pageNum}
								</PaginationLink>
							</motion.div>
				...
				<PaginationItem>
					<motion.div initial="initial" animate="animate" exit="exit" whileHover="hover" variants={itemVariants} transition={{ duration: 0.3 }}>
						<PaginationNext
							...
							className={cn(currentPage >= totalPages ? 'pointer-events-none opacity-50' : '', variant === 'rounded' ? 'rounded-full' : '')}
						/>
					</motion.div>
				</PaginationItem>
			</PaginationContent>
					...
					>
						<div className="mb-4 flex items-center justify-between border-b pb-2">
							<h4 className="text-muted-foreground text-sm font-medium">
								Showing {startIndex + 1}-{Math.min(startIndex + itemsPerPage, demoItems.length)} of {demoItems.length} items
							</h4>
								...
								>
									<div>
										...
										<div className="text-muted-foreground flex items-center gap-2 text-sm">
											<span className="bg-primary/10 text-primary rounded-full px-2 py-0.5 text-xs font-medium">{item.category}</span>
											<span>{item.date}</span>
										</div>
									</div>
							...
							<PaginationItem>
								<motion.div initial="initial" animate="animate" exit="exit" whileHover="hover" variants={itemVariants} transition={{ duration: 0.3 }}>
									<PaginationPrevious
										...
										className={cn(page <= 1 ? 'pointer-events-none opacity-50' : '', variant === 'rounded' ? 'rounded-full' : '')}
									/>
								</motion.div>
							</PaginationItem>
									...
									if ((pageNum === 2 && page > 3) || (pageNum === totalPages - 1 && page < totalPages - 2)) {
										return (
											<PaginationItem key={`ellipsis-${pageNum}`}>
												<motion.div initial="initial" animate="animate" exit="exit" variants={itemVariants} transition={{ duration: 0.3 }}>
													<PaginationEllipsis />
												</motion.div>
											</PaginationItem>
										);
							...
							<PaginationItem>
								<motion.div initial="initial" animate="animate" exit="exit" whileHover="hover" variants={itemVariants} transition={{ duration: 0.3 }}>
									<PaginationNext
										...
										className={cn(page >= totalPages ? 'pointer-events-none opacity-50' : '', variant === 'rounded' ? 'rounded-full' : '')}
									/>
								</motion.div>
							</PaginationItem>
						</PaginationContent>
			...
			<div className="mx-auto max-w-6xl px-4 py-12">
				<div className="mb-12 text-center">
					...
					<p className="text-muted-foreground mx-auto max-w-2xl">
						Interactive pagination components that demonstrate real content navigation. Each example shows how pagination can be used to navigate through a collection
						of items.
					</p>
				</div>
				...
				<div className="space-y-16">
					{/* Default Variant */}
					<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="bg-card rounded-lg border p-6 shadow-sm">
		...
		<div className="flex items-center justify-between">
			...
			>
				<SelectTrigger className="w-[140px]">
					<div>
						<span className="text-muted-foreground mr-2">{t('view')}: </span>
						<SelectValue />
					</div>
				</SelectTrigger>
				<SelectContent>
					<SelectGroup>
						<SelectLabel>{t('sizes')}</SelectLabel>
						{sizes.map((size) => (
